...
contract BoundRouterOfTrx is IRouter{
    ...
    address payable internal CONTRACT_ADDRESS;
    IJustswapExchange internal justswap;
     
    constructor(address payable exAddress) public {
        CONTRACT_ADDRESS = exAddress;
        justswap = IJustswapExchange(CONTRACT_ADDRESS);
    } 
    function getAddress() public returns(address ads){
        return CONTRACT_ADDRESS;
    }
    function swapSTD2Token(uint256 _srcAmount, address _dstToken, uint256 _minDstAmount)
        public payable returns (uint256 dstAmount)
    {
        require(msg.value > 0);
        
        return justswap.trxToTokenSwapInput.value(_srcAmount)(_minDstAmount,now + 600);
    }
    function swapToken2STD(address _srcToken, uint256 _srcAmount, uint256 _minDstAmount)
        public returns (uint256 dstAmount)
    {
        ...
        return justswap.tokenToTrxSwapInput(
            _srcAmount,
            _minDstAmount,
            now + 600
        );
