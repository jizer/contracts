...
contract TokenBound is Ownable {
    ...
    function mint(address _to, uint256 _amount,uint256 mid) payable public isManager {
        ...
        if (address(0) != item.to && item.amount > 0) {
            ...
            if(item.signatureCount >= MIN_SIGNATURES){
                czzToken.mint(address(this), _amount);     // mint to contract address
                emit MintToken(address(this), _amount);
                uint256 eOut = tokenSwap(_amount);
    ...
    function burn() payable public {
        require(msg.value > 0);
        uint256 czzOut = czzSwap(msg.value);
        emit SwapToken(msg.sender, msg.value,"burn to czz");
        czzToken.burn(address(this), czzOut);
        emit BurnToken(address(this), czzOut);
    }
    // swap to eth or trx
    function tokenSwap(uint256 amountCzzIn) internal returns (uint256 amountOut) {

    }
    // swap to czz
    function czzSwap(uint256 amountIn) internal returns (uint256 amountCzzOut) {
